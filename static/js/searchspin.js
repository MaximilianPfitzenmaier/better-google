    ...
    // variable needed for calculation animations and handlers
    ...
    // value options for dragging and scrolling speed
 
    const speedWheel = 0.008
    const speedDrag = -0.1
    

    // generate z to calculate animation range
    ...
    function updateDonutChart(chart, percentage) {
        ...
        if (!isNaN(currentPercentage) && currentPercentage >= 0 && currentPercentage <= 100) {
          ...
          circleFill.style.transition = 'stroke-dashoffset 0.1s ease-in-out';
          updateChart();
        }
      }
    ...
    for (let mutation of mutationsList) {
        ...
        if (target.classList.contains('middle')) {
            ...
            // Check if the div with the data Attribute exists if yes create a bar chart
            if(barchartdiv){
    ...
    //clickevents  for the scroll animation
    ...
    // eventhandlers for the scroll animation
    ...
    if (document.getElementById("searched_query")) { // Check if Query exists
      var query = document.getElementById("searched_query").textContent; 
  
      // If Query exists show it inside Details
      var paragraph = document.getElementById("searchquery");
      if(paragraph){
